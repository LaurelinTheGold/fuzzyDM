...
#Example file name: avgBrightTemp_CDM.txt
#make sure to write the brightness temperature file names to a text doc
#in Boxes, "ls delta_T_* > avgBrightTemp_XDM.txt"

#latex
plt.rcParams.update({"text.usetex": True, "font.size": 16})

#This code takes in a folder of filename files and graphs avg brightness temp vs z
...
axe.set_xlabel(r'$z$')
axe.set_ylabel(r'$\langle T_{\mathrm{Bright}} \rangle$')
...
for file in name: #foreach file matching name in directory
	...
	#CDM/FDM
	dmTemp = parsedFileName[1].rpartition('.') #Checks for .txt on end
	if (dmTemp[-1] == "txt"): #remove if present
		dm = dmTemp[0]
	else: #else ignore
		dm = parsedFileName[1]
	z = []
	...
	for j in range(91): #grabs the data
		...
		z.append(getZ)
		avgBrtTmp.append(getABT)
	#graphy boi with labels
	plt.plot(z, avgBrtTmp, label=(dm+": "+r"$T_{\mathrm{vir}} = 2 \times 10^4$ K"+", $\zeta = 20$"), 
...
# plt.show()
plt.savefig("/Users/richardchen/Desktop/summerResearch/2020summer/fiducialBrightTemp/completeAvgBrightTemp.pdf") #save